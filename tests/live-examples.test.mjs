...
const view = (id, label, base_table, paths) => ({
  attributes: { drupal_internal__id: id, label, base_table, display: Object.fromEntries(paths.map((path, i) => [`d${i}`, { display_options: { path } }])) },
})
...
describe('SOURCES', () => {
  ...
  beforeEach(() => {
    ...
    globalThis.fetch = async (url) => {
      ...
      if (url.includes('/view/view')) {
        return jsonapi([
          view('admin_audit_trail', 'Admin Audit Trail', 'admin_audit_trail', ['admin/reports/audit-trail']),
      ...
      if (url.includes('/node/recipe')) return jsonapi([{ attributes: { title: 'Borscht', path: { alias: '/recipes/borscht' } } }, { attributes: { title: 'No alias', path: {} } }])
      if (url.includes('/configurable_language/')) return { ok: false, status: 404, json: async () => ({}) }
      if (url.includes('/menu/menu')) return jsonapi([{ attributes: { drupal_internal__id: 'main', label: 'Main navigation' } }])
  ...
  test('views keeps content views without an admin path, sorted by label', async () => {
    ...
    assert.deepEqual(list.map((o) => o.value), ['frontpage', 'recipes'])
  ...
  test('menus are labelled the way the Storybook stories name them', async () => {
    assert.deepEqual(await m.SOURCES.menus('/jsonapi'), [{ value: 'main', label: 'Main navigation (main)' }])
  })
})
...
describe('COMPONENTS', () => {
  test('every live component has a package and an API page', () => {
    for (const name of m.COMPONENT_NAMES) assert.match(m.pageFor(name), new RegExp(`^/api/packages/${m.PACKAGES[name]}/components/${name}$`))
  })
  ...
  test('selects prefer content: nodes, the site menu, the frontend theme of each backend', () => {
    ...
    assert.equal(theme.prefer({ backend: 'site', $config: { decoupledTheme: { default: 'druxtjs' } } }), 'druxtjs')
  })
