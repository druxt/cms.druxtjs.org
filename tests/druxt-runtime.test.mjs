...
describe('schemaImporter', () => {
  test('returns the schema a display gives', async () => {
    const importer = schemaImporter(builder({ 'node--recipe--full--view': { id: 'node--recipe--full--view', fields: [{ id: 'title' }] } }))
    assert.deepEqual(await importer('node--recipe--full--view'), { id: 'node--recipe--full--view', fields: [{ id: 'title' }] })
  })

  test('falls back to the default mode when the mode has no display', async () => {
    const importer = schemaImporter(builder({ 'node--recipe--default--view': { id: 'node--recipe--default--view', fields: [] } }))
  ...
  test('gives a bundle with no display at all an empty schema rather than nothing', async () => {
    ...
    assert.deepEqual(schema, { id: 'file--file--default--view', resourceType: 'file--file', config: { entityType: 'file', bundle: 'file', mode: 'default', schemaType: 'view', filter: [] }, fields: [] })
  })

  test('treats a builder that throws the same as one that finds nothing', async () => {
    const throwing = { druxt: builder({}).druxt, getSchema: async () => { throw new Error('403') } }
...
describe('rerouteFiles', () => {
  test('prefixes relative file URLs in data and included with the proxy root, and nothing else', () => {
    const response = { data: { data: [{ type: 'file--file', attributes: { uri: { url: '/sites/default/files/a.jpg' } } }], included: [
