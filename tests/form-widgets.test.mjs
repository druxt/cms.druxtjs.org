...
describe('numberRange', () => {
  test('spans the configured range', () => {
    assert.deepEqual(w.numberRange({ settings: { config: { min: -2, max: 2 } } }, 0), [-2, -1, 0, 1, 2])
  })
...
describe('allowedOptions', () => {
  test('takes a list of value and label, or a map of value to label', () => {
    assert.deepEqual(w.allowedOptions({ settings: { storage: { allowed_values: [{ value: 'a', label: 'A' }] } } }), [{ value: 'a', label: 'A' }])
    assert.deepEqual(w.allowedOptions({ settings: { config: { allowed_values: { b: 'B' } } } }), [{ value: 'b', label: 'B' }])
    assert.deepEqual(w.allowedOptions({}), [])
  })
})

describe('references', () => {
  const schema = { settings: { storage: { target_type: 'taxonomy_term' }, config: { handler_settings: { target_bundles: { tags: 'tags', section: 'section' } } } } }
