...
Every environment except production sends `X-Robots-Tag: noindex, nofollow`.
...
- `sitemap.xml`, `llms.txt` and the share cards are only written by
  `nuxt generate`, so this server does not have them yet.
